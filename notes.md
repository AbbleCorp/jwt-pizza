...
| Order pizza                                         |        menu.tsx       |        [GET]/api/order/menu<br>[GET]/api/franchise?page=0&limit=10&name=*        |      `SELECT userId FROM auth WHERE token=?`(checking if logged in)<br>`SELECT * FROM menu`<br>`SELECT id, name FROM franchise WHERE name LIKE ? LIMIT ${limit + 1} OFFSET ${offset}`<b>`SELECT id, name FROM store WHERE franchiseId=?`<br>     |
| Verify pizza                                        |                    |                   |              |
| View profile page                                   |                    |                   |              |

...
| Order pizza                                         |        menu.tsx       |        [GET]/api/order/menu<b>[GET]/api/franchise?page=0&limit=10&name=*        |      `SELECT userId FROM auth WHERE token=?`(checking if logged in)<b>`SELECT * FROM menu`<b>`SELECT id, name FROM franchise WHERE name LIKE ? LIMIT ${limit + 1} OFFSET ${offset}`<b>`SELECT id, name FROM store WHERE franchiseId=?`<b>     |
| Verify pizza                                        |                    |                   |              |
| View profile page                                   |                    |                   |              |
